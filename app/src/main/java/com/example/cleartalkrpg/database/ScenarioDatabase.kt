...
abstract class ScenarioDatabase: RoomDatabase() {
    ...
    private class ScenarioDatabaseCallback(
        ...
        suspend fun populateDatabase(context: Context, scenarioDao: ScenarioDao) {
            /* シードデータの挿入 */
            val jsonString = context.resources.openRawResource(R.raw.scenarios).use(InputStream::readBytes).decodeToString()
            val scenarios: List<Scenario> = Json.decodeFromString(jsonString)
